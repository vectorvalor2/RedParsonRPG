...
export type AssetMeta = {
  ...
  tags: string[];
  sizeBytes: number;
  provenance: "local" | "template" | "ai";
  complexity: number; // 0..1 (polys, materials, skeleton, scripts)
  unsafeHints?: string[]; // optional red flags
};
...
export class AssetRegistry {
  ...
  computeConfidence(meta: AssetMeta): ConfidenceScore {
    ...
    const prov = meta.provenance === "template" ? 1 : meta.provenance === "local" ? 0.85 : 0.65;
    const complexity = 1 - meta.complexity; // simpler = safer
    const size = Math.max(0, 1 - meta.sizeBytes / (5_000_000)); // 5MB soft cap
